...
def run_alert_check() -> bool:
    ...
    - Look back 24 hours to catch source timestamps that lag ingestion time
    - Include HIGH and MEDIUM priority items
    - Double-check alert_sent before and after sending
    - Mark entries as alerted after a successful send
    ...
    print(f"Found {len(new_alerts)} new high-priority events to alert:")
    for u in new_alerts[:5]:
        print(f"  [{u.get('impact_level', '?').upper()}] {u.get('title', '')[:80]}")
    ...
    try:
        ...
        if success:
            ...
            logger.info("Marked %d entries as alerted", len(new_alerts))
        else:
